...
| `meetup_participants` | 약속 참여자 (status, share_location 토글) |
...
마이그레이션은 `supabase/migrations/001~013`에 순서대로 정의되어 있고, `009_rls_policies.sql`이 정책 본문입니다.
...
### 7.3 푸시 알림 (Edge Function + pg_cron)
